...
- Interface CLI : `yt [URL | fichier.txt] [--dry-run] [--gist]`
...
https://youtu.be/yyyy model=claude-haiku-4-5  ← surcharge modèle par URL
https://youtu.be/zzzz gist        ← publie le gist pour cette URL uniquement
# commentaire ignoré
                                  ← ligne vide ignorée
```

Priorité modèle : ligne URL > `# model:` fichier > `config.yml`  
Priorité gist : `gist` sur la ligne URL > `# gist` entête fichier  
Si `# gist` est absent et qu'aucune ligne n'a `gist` : aucune publication.
...
```
Mode unitaire sans --gist (inchangé) :
...
1-15. Pipeline ci-dessus
16.   Vérification gh disponible et authentifié → erreur lisible si absent, fiche locale préservée
17.   Publication gist secret depuis le fichier écrit à l'étape 14
18.   Confirmation terminal : chemin fichier + URL gist

Mode unitaire avec --gist, fiche déjà existante :
1.    Détection fiche existante dans le vault
2.    Pas de régénération
3.    Vérification gh disponible et authentifié → erreur lisible si absent
4.    Publication gist secret depuis la fiche existante
